...
// ConnectUsingProtocolWithTLS creates a secure OVSDB connection and returns and OvsdbClient
func ConnectUsingProtocolWithTLS(protocol string, target, privateKeyPath, publicKeyPath, caPath string) (*OvsdbClient, error) {
	...
	conn, err := tls.Dial(protocol, target, tlsConfig)
	if err != nil {
		return nil, err
	}
