...
openssl req -newkey rsa:4096 -nodes -keyout ca.key -subj "/C=AT/ST=Vienna/L=Vienna/O=Test/OU=Test/CN=ca.gnxi.lan"
# Generate CA Certifiacate Signing Request
openssl req -key ca.key -new -out ca.csr -subj "/C=AT/ST=Vienna/L=Vienna/O=Test/OU=Test/CN=ca.gnxi.lan"
...
openssl req -newkey rsa:4096 -nodes -keyout faucet.key -subj "/C=AT/ST=Vienna/L=Vienna/O=Test/OU=Test/CN=faucet.gnxi.lan"
# Generate Controller Certificate Signing Request
openssl req -key faucet.key -new -out faucet.csr -subj "/C=AT/ST=Vienna/L=Vienna/O=Test/OU=Test/CN=faucet.gnxi.lan"
# Generate and sign Controller Certificate by CA
openssl x509 -req -in faucet.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out faucet.crt

# Generate Target Private Key
openssl req -newkey rsa:4096 -nodes -keyout target.key -subj "/C=AT/ST=Vienna/L=Vienna/O=Test/OU=Test/CN=target.gnxi.lan"
# Generate Target Certificate Signing Request
openssl req -key target.key -new -out target.csr -subj "/C=AT/ST=Vienna/L=Vienna/O=Test/OU=Test/CN=target.gnxi.lan"
...
openssl req -newkey rsa:4096 -nodes -keyout client.key -subj "/C=AT/ST=Vienna/L=Vienna/O=Test/OU=Test/CN=client.gnxi.lan"
# Generate Client Certificate Signing Request
openssl req -key client.key -new -out client.csr -subj "/C=AT/ST=Vienna/L=Vienna/O=Test/OU=Test/CN=client.gnxi.lan"
